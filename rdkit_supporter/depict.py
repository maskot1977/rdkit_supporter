...
def classification_metrics(model, X, Y):
    Y_pred = model.predict(X)
    scores = []
    scores.append(["MCC", metrics.matthews_corrcoef(Y_pred, Y2)])
    scores.append(["Cohen's Kappa", metrics.cohen_kappa_score(Y_pred, Y2)])
    scores.append(["F1", metrics.f1_score(Y_pred, Y2)])
    scores.append(["Average Precision", metrics.average_precision_score(Y_pred, Y2)])
    scores.append(["Precision", metrics.precision_score(Y_pred, Y2)])
    scores.append(["Recall", metrics.recall_score(Y_pred, Y2)])
    scores.append(["AUC", metrics.balanced_accuracy_score(Y_pred, Y2)])
    #scores.append(["TopK ACC", metrics.top_k_accuracy_score(Y_pred, Y2)])
    scores.append(["Balanced ACC", metrics.balanced_accuracy_score(Y_pred, Y2)])
    scores.append(["ACC", metrics.accuracy_score(Y_pred, Y2)])
